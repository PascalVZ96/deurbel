...
(() => {
  ...
  window.addEventListener('pointerdown', () => { interactionSeen = true; }, {once:true, passive:true});
  window.addEventListener('keydown', () => { interactionSeen = true; }, {once:true});
  ...
  function row(label, value, ok = null){
    ...
    return '<div class="pwa-diag-row"><span class="pwa-diag-state '+state+'">'+icon+'</span><span class="pwa-diag-label">'+escapeHtml(label)+'</span><strong>'+escapeHtml(value)+'</strong></div>';
  }

  async function collect(){
    const result = {};
    result.secureContext = window.isSecureContext;
    result.standalone = !!(window.matchMedia?.('(display-mode: standalone)').matches || navigator.standalone === true);
    result.installPromptSeen = installPromptSeen || window.__securityPwaInstallPromptSeen === true;
    ...
    try {
      const response = await fetch(manifestLink?.href || '/pwa/manifest.webmanifest', {cache:'no-store', credentials:'include'});
  ...
  function ensurePanel(){
    ...
    panel.innerHTML = `
      <div class="pwa-diag-head">
        <div><div class="pwa-diag-eyebrow">Chrome / Android</div><h2>PWA-diagnose</h2></div>
    ...
    style.textContent = `
      ...
      .pwa-diag-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.pwa-diag-head h2{margin:2px 0 12px;font-size:22px}.pwa-diag-eyebrow{font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:#8b9bb0;font-weight:800}.pwa-diag-head button{border:1px solid #344155;background:#171f2b;color:#fff;border-radius:10px;width:36px;height:36px;font-size:24px}.pwa-diag-row{display:grid;grid-template-columns:24px minmax(110px,1fr) minmax(120px,1.4fr);gap:8px;align-items:start;padding:9px 0;border-bottom:1px solid #202b39;font-size:12px}.pwa-diag-row strong{overflow-wrap:anywhere;text-align:right}.pwa-diag-state{font-weight:900}.pwa-diag-state.ok{color:#75d89a}.pwa-diag-state.bad{color:#ff7e8d}.pwa-diag-state.neutral{color:#90a0b4}.pwa-diag-label{color:#9cabbc}.pwa-diag-note{margin:12px 0 0;padding:11px;border-radius:12px;background:#151e29;color:#afbdcc;font-size:11px;line-height:1.5}.pwa-diag-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px}.pwa-diag-actions button{min-height:42px;border-radius:11px;border:1px solid #344155;background:#1a2532;color:#eef4fb;font-weight:750}
      html[data-theme="light"] #pwaDiagnostics{background:#fff;color:#19334b;border-color:#cdd9e3}.pwa-diag-row{border-color:#dfe7ee}html[data-theme="light"] .pwa-diag-note{background:#edf4f9;color:#496276}html[data-theme="light"] .pwa-diag-actions button,html[data-theme="light"] .pwa-diag-head button{background:#edf4f9;color:#19334b;border-color:#cdd9e3}
    `;
    ...
    document.getElementById('pwaDiagCopy').onclick = async () => {
      ...
      catch { prompt('Kopieer dit resultaat:', text); }
    };
    return panel;
  }
  ...
  async function render(panel = ensurePanel()){
    ...
    const has192 = sizes.some(value => String(value).split(/\\s+/).includes('192x192'));
    const has512 = sizes.some(value => String(value).split(/\\s+/).includes('512x512'));
    const rootRegistration = (data.registrations || []).find(reg => reg.scope === location.origin + '/');
    ...
    ].join('') + '<div class="pwa-diag-note">De beslissende regel is <strong>beforeinstallprompt</strong>. Als alle manifestregels groen zijn maar die na een tik en 30+ seconden niet afgaat, ligt de blokkade aan Chrome/apparaat-installatie en niet meer aan het Security Center.</div>';
  }
